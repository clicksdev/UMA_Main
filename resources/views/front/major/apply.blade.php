    ...
    {{-- <div>
        ...
        <div class="page_path">
            <div class="container">
                ...
                <span>/</span>
                <a href="/major/apply/{{$major->id}}">Apply</a>
            </div>
        </div>
        ...
        <section class="form" id="apply_wrapper">
            <div class="container">
                <form ref="form" @submit.prevent="handleSubmit" action="{{route('major.apply')}}" enctype="multipart/form-data" method="POST" style="background: #ECECEC; width: 100%">
                    @csrf
                    <input type="hidden" name="course" value="{{$major->title . " - " . $major->patch}}">
                    ...
                    <div class="step-3" :class="step == 3 ? '' : 'hide'">
                        <h2>Task</h2>
                        @foreach ($major->questions as $item)
...
@section("script")
    ...
    <script src="{{asset('/assets/vueJs/MajorApply.js')}}?={{time()}}"></script>
    <script>
