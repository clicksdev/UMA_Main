...
@section('content')
    ...
    <section class="presedentSection">
        <div class="container">
            <div class="img">
                ...
                <div class="breif">
                    <h1>
                        {{ $settingsArray['presedent_name' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}
                    </h1>
                    <p>
                        {{ $settingsArray['presedent_position' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}
                    </p>
                </div>
            </div>
            <div class="text">
                ...
                <p>
                    {!! $settingsArray['presedent_word_part' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] !!}
                </p>
                <a href="/president-word">{{ __('home.read_biography') }}</a>
            </div>
        </div>
    </section>
    @endif
            ...
            <div class="body" style="margin-top: 0">
                <div class="items_wrapper">
                    <a href="" class="item-card" style="min-width: 100%">
                        <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-book" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#000000" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                        ...
                        <h2 style="margin-bottom: 12px !important">{{ __('home.uma_vision') }}</h2>
                        <p>
                            {!! $settingsArray['uma_vision' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] !!}
                        </p>
                    </a>
                    <a href="" class="item-card" style="min-width: 100%">
                        <div class="icon">
                        ...
                        <h2 style="margin-bottom: 12px !important">{{ __('home.uma_mission') }}</h2>
                        <p>
                            {!! $settingsArray['uma_mission' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] !!}
                        </p>
                    </a>
                </div>
            </div>
    ...
    @if($latest_courses->count() > 0)
        ...
        <section class="our-major" id="our-major">
            <div class="container">
                <div class="head">
                    <img src="{{ asset('front/assets/imgs/sec-head.png') }}?v={{ time() }}" alt="{{ __('home.logo_short') }}">
                    <h1>
                        {{ __('home.our') }}
                        <br>
                        <span>
                    ...
                    <div class="swiper mySwiper2 sipermaj" style="width: 100%">
                        <div class="swiper-wrapper">
                            ...
                            <a href="{{$course->isReady ? '/course/' . $course->id : "#"}}" class="course_card swiper-slide {{$course->isReady ? '' : 'coming-soon'}}">
                                <div class="img">
                                        <img src="{{ $course->hasMedia() ? $course->getFirstMediaUrl() : '' }}" alt="{{ __('home.course_image_alt') }}">
                                        <span class="date">
                                            ...
                                            {{ __('home.start') }} {{ Carbon\Carbon::parse($course->started_at)->format(__('home.date_format')) }}
                                        </span>
                                    </div>
                                    <div class="text">
                                        <h3>{{ $course['title' . (App::getLocale() == 'ar' ? '_ar' : '')] }}</h3>
                                        <span class="time">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-clock-filled" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#000000" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    ...
    @php
        $latest_work_shops = App\Models\Course::withoutGlobalScope('excludeWorkShop')->where('course_type', 'work_shop')->orderBy("home_arrangment", "asc")->where("doesShow", true)->get();
    @endphp
    @if($latest_work_shops->count() > 0)
 ...
 --}}
    ...
    <section class="our-aim">
        <div class="container">
            <div class="head">
                <div class="text">
                    <span>{{ __('home.our_aim') }}</span>
                    <h1>{{ $settingsArray['our_aim_title' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</h1>
                    <p>{{ $settingsArray['our_aim_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                </div>
                <form action="">
                    <h3>{{ __('home.what_we_offer') }}</h3>
                    <button class="active">
            ...
            <div class="body">
                <div class="items_wrapper">
                    <a href="" class="item-card">
                        ...
                        <h2>{{ __('home.main_programs') }}</h2>
                        <p>{{ $settingsArray['main_majors_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </a>
                    <a href="" class="item-card">
                        <div class="icon">
                        ...
                        <h2>{{ __('home.online_courses') }}</h2>
                        <p>{{ $settingsArray['online_courses_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </a>
                    <a href="" class="item-card">
                        <div class="icon">
                        ...
                        <h2>{{ __('home.masterclass') }}</h2>
                        <p>{{ $settingsArray['masterclass_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </a>
                </div>
            </div>
        </div>
    ...
    <section class="featured-courses">
        <div class="container">
            <div class="head">
                <span>{{ __('featured_courses.title') }}</span>
                <h1>{{ __('featured_courses.subtitle') }}</h1>
                <p>{{ __('featured_courses.description') }}</p>
            </div>
            <div class="courses_wrapper">
                <a href="" class="course-card">
                    <div class="img">
                        <img src="{{ asset('front/assets/imgs/courses/course-8.jpeg') }}"
                             alt="{{ __('featured_courses.course_image_alt') }}">
                    </div>
                    <span class="cat">{{ __('featured_courses.category_writing') }}</span>
                    <h3>{{ __('featured_courses.course_title_scenario_writing') }}</h3>
                    <div class="details">
                        <span>12 {{ __('featured_courses.video_count') }}</span>
                        <span>24 {{ __('featured_courses.hours_minutes', ['hours' => 24, 'minutes' => 40]) }}</span>
                    </div>
                </a>
                <a href="" class="course-card">
                    <div class="img">
                        <img src="{{ asset('front/assets/imgs/courses/course-9.jpeg') }}"
                             alt="{{ __('featured_courses.course_image_alt') }}">
                    </div>
                    <span class="cat">{{ __('featured_courses.category_digital_content') }}</span>
                    <h3>{{ __('featured_courses.course_title_social_media') }}</h3>
                    <div class="details">
                        <span>8 {{ __('featured_courses.video_count') }}</span>
                        <span>12 {{ __('featured_courses.hours_minutes', ['hours' => 12, 'minutes' => 10]) }}</span>
                    </div>
                </a>
                <a href="" class="course-card">
                    <div class="img">
                        <img src="{{ asset('front/assets/imgs/courses/course-3.jpeg') }}"
                             alt="{{ __('featured_courses.course_image_alt') }}">
                    </div>
                    <span class="cat">{{ __('featured_courses.category_digital_content') }}</span>
                    <h3>{{ __('featured_courses.course_title_short_videos') }}</h3>
                    <div class="details">
                        <span>4 {{ __('featured_courses.video_count') }}</span>
                        <span>5 {{ __('featured_courses.hours_minutes', ['hours' => 5, 'minutes' => 40]) }}</span>
                    </div>
                </a>
            </div>
            {{--<a href="" class="explore">{{ __('featured_courses.explore_all') }}</a>--}}
        </div>
    </section>
    @endif
    ...
    <section class="our-aim how-it-work">
        <div class="container">
            <div class="head">
                <div class="text">
                    <span>{{ __('how_it_works.section_title') }}</span>
                    <h1>{!! $settingsArray['how_it_works_title' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] !!}</h1>
                    <p>{{ $settingsArray['how_it_works_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                </div>
            </div>
            <div class="body">
                <div class="statistics_wrapper">
                    <div class="step-card">
                        <div class="head">
                            <h1>{{ __('how_it_works.step_one_number') }}</h1>
                            <div class="line"></div>
                        </div>
                        <h1>{{ $settingsArray['how_it_works_step_one_title' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</h1>
                        <p>{{ $settingsArray['how_it_works_step_one_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </div>
                    <div class="step-card">
                        <div class="head">
                            <h1>{{ __('how_it_works.step_two_number') }}</h1>
                            <div class="line"></div>
                        </div>
                        <h1>{{ $settingsArray['how_it_works_step_two_title' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</h1>
                        <p>{{ $settingsArray['how_it_works_step_two_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </div>
                    <div class="step-card">
                        <div class="head">
                            <h1>{{ __('how_it_works.step_three_number') }}</h1>
                            <div class="line"></div>
                        </div>
                        <h1>{{ $settingsArray['how_it_works_step_three_title' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</h1>
                        <p>{{ $settingsArray['how_it_works_step_three_description' . (App::getLocale() == 'ar' ? '_ar' : '')]['value'] }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container">
                    ...
                    @foreach($testimonials as $testimonial)
                        <div class="swiper-slide">
                            <div class="img">
                                ...
                                <div class="text">
                                    {{ $testimonial['username' . (App::getLocale() == 'ar' ? '_ar' : '')] }}
                                    <span>
                                    {{ $testimonial['name' . (App::getLocale() == 'ar' ? '_ar' : '')] }}
                                </span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="text_wrapper">
                ...
                <div class="test-card {{ $loop->first ? 'active' : '' }}">
                    ...
                    <h4>{{ $testimonial['title' . (App::getLocale() == 'ar' ? '_ar' : '')] }}</h4>
                    <p>{{ $testimonial['description' . (App::getLocale() == 'ar' ? '_ar' : '')] }}</p>
                </div>
            @endforeach

            <a href="">{{ __('home.read_more_testimonials') }}</a>
        </div>
        </div>
    </section>
    @endif
    ...
    <section class="featured-courses subscribe">
        <div class="container">
            <div class="head">
                <span>{{ __('home.never_miss_update') }}</span>
            ...
            <div class="body">
                <div class="items_wrapper">
                    <div class="item">
                        <h1>
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-check-filled" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#000000" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                    ...
                    <div class="item">
                        <h1>
                            ...
                            {{ __('home.online_courses') }}
                        </h1>
                        <span>Sed purus molestie.</span>
                    </div>
                </div>
                <form id="emailForm">
                    <input type="email" name="email" id="email" placeholder="{{ __('home.email_placeholder') }}">
                    <button>{{ __('home.get_started_now') }}</button>
                </form>

                <div class="note">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-lock" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#000000" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
...
<!-- First modal (Signup Success) -->
<div class="modal fade" id="signupSuccessModal" tabindex="-1" role="dialog" aria-labelledby="signupSuccessModalLabel"
     aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="signupSuccessModalLabel">{{ __('modal.success') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                {{ __('modal.signup_success_message') }}
            </div>
        </div>
    </div>
</div>

<!-- Second modal (Subscription Success) -->
<div class="modal fade" id="subscribeSuccessModal" tabindex="-1" role="dialog" aria-labelledby="subscribeSuccessModalLabel"
     aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="subscribeSuccessModalLabel">{{ __('modal.success') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                {{ __('modal.subscribe_success_message') }}
            </div>
        </div>
    </div>
</div>
